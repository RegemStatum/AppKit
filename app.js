...
window.onscroll = function(e) {
    ...
    if((window.pageYOffset > mainBlocks.about.offsetTop-3) && (window.pageYOffset <= mainBlocks.testimonials.offsetTop-3) ) {
        headerList.forEach((item)=>{item.classList.remove("li-chosen")})
        headerList.item(0).classList.add("li-chosen")
    } else if((window.pageYOffset > mainBlocks.testimonials.offsetTop-3) && (window.pageYOffset <= mainBlocks.features.offsetTop-3) ) {
        headerList.forEach((item)=>{item.classList.remove("li-chosen")})
        headerList.item(1).classList.add("li-chosen")
    } else if((window.pageYOffset > mainBlocks.features.offsetTop-3) && (window.pageYOffset <= mainBlocks.team.offsetTop-3) ) {
        headerList.forEach((item)=>{item.classList.remove("li-chosen")})
        headerList.item(2).classList.add("li-chosen")
    } else if((window.pageYOffset > mainBlocks.team.offsetTop-3) && ( window.pageYOffset <= mainBlocks.pricing.offsetTop-3)) {
        headerList.forEach((item)=>{item.classList.remove("li-chosen")})
        headerList.item(3).classList.add("li-chosen")
    } else if((window.pageYOffset > mainBlocks.pricing.offsetTop-3) && (window.pageYOffset <= (mainBlocks.contact.offsetTop-3 - mainBlocks.contact.offsetHeight/0.8))) {
        headerList.forEach((item)=>{item.classList.remove("li-chosen")})
        headerList.item(4).classList.add("li-chosen")
    } else if(window.pageYOffset > (mainBlocks.contact.offsetTop-3 - mainBlocks.contact.offsetHeight/0.8)) {
        headerList.forEach((item)=>{item.classList.remove("li-chosen")})
...
function changeTopBg(index) {
    ...
    markers.forEach((marker)=>{marker.classList.remove("marker-highlighted")})
    ...
    setTimeout(()=> {
        ...
        topBgButton.classList.remove("fade")}, 600)
}
...
for(let a of as) {
    a.addEventListener("click", (e)=>{
        as.forEach((a)=>{a.classList.remove("features-list-active")})
        a.classList.add("features-list-active")
        e.preventDefault()
        let index = Array.prototype.indexOf.call(as, a)
        image.parentElement.classList.add("hidden")
        setTimeout(()=>{image.src = `images/feature-${index+1}.png`
                               image.parentElement.classList.remove("hidden")}, 400)
    })
}
...
headerMenuButton.addEventListener("click", (e)=>{
    if(headerMenu.classList.contains("mobile")) {
        ...
        setTimeout(()=>{headerMenu.classList.toggle("shown")}, 0)
    }
})
